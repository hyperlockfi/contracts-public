...
contract Booster is ReentrancyGuard {
    ...
    /**
     * @notice Fee Manager can update the fees (lockIncentive, stakeIncentive, earmarkIncentive)
     */
    ...
    function setRewardContracts(address _rewards, address _stakerRewards) external {
        ...
        //reward contracts are immutable
    ...
    /**
     * @notice Set the booster fee distro address
     */
    function setBoosterFeeDistro(address _distro) external {
        require(msg.sender == owner, "!auth");
        boosterFeeDistro = _distro;
        emit BoosterFeeDistroUpdated(_distro);
    }
    ...
    function setFeeInfo(address _feeToken, address _feeDistro) external nonReentrant {
        ...
        if (feeTokens[_feeToken].distro == address(0)) {
            ...
            feeTokens[_feeToken] = FeeDistro({ distro: _feeDistro, rewards: lockRewards, active: true });
            emit FeeInfoUpdated(_feeDistro, lockRewards, _feeToken);
        } else {
     ...
     * @param _stakerFees   % for CVX lockers where 1% == 100
    ...
    function setFees(
        ...
        require(_lockFees <= MaxFees, "!lockFees");
        ...
        emit FeesUpdated(_lockFees, _stakerFees, _callerFees);
    }
    ...
    function _calculateIncentives(uint256 rewardAmount)
        internal
        view
        returns (
            uint256 _lockIncentive,
        ...
        )
    {
        ...
        _lockIncentive = rewardAmount.mul(lockIncentive).div(FEE_DENOMINATOR);
        // StakerIncentive = cvx stakers (currently 5%)
        _stakerIncentive = rewardAmount.mul(stakerIncentive).div(FEE_DENOMINATOR);
        // CallIncentive = caller of this contract (currently 1%)
        _callIncentive = rewardAmount.mul(earmarkIncentive).div(FEE_DENOMINATOR);
        // Sum of all incentives = lock + staker + call
        _totalIncentive = _lockIncentive.add(_stakerIncentive).add(_callIncentive);

        //remove incentives from balance
        _rewardIncentive = rewardAmount.sub(_totalIncentive);
    }
    ...
    {
        return _calculateIncentives(rewardAmount);
    }
