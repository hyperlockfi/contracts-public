...
contract GaugeVoter is LzApp {
    ...
    event SetAuraOFT(address auraOFT);
    ...
    /**
     * @dev Set AURA OFT contract address
     * @param _auraOFT AURA OFT contract
     */
    function setAuraOFT(address _auraOFT) external onlyOwner {
        auraOFT = IOFT(_auraOFT);

        IERC20(aura).safeApprove(_auraOFT, type(uint256).max);

        emit SetAuraOFT(_auraOFT);
    }
    ...
    function processSidechainGaugeRewards(
        ...
        require(address(auraOFT) != address(0), "!auraOFT");
