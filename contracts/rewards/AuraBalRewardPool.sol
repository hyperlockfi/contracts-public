...
contract AuraBalRewardPool {
    ...
    constructor(
        ...
        require(_compounder != address(0), "!compounder");
        compounder = IERC4626(_compounder);
    ...
    function withdraw(
        ...
        if (migrate) {
            require(address(compounder) != address(0), "!compounder");
            stakingToken.safeApprove(address(compounder), 0);
            stakingToken.safeApprove(address(compounder), amount);
            compounder.deposit(amount, msg.sender);
